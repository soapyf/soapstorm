...
#include "ssatmoenvapplier.h" // <SS:Nexii> horizon-clip gated celestial disc cull
...
// Texture coordinates:
    ...
    bool ssQuadClearsHorizon(F32 scale, F32 disk_radius, const LLVector3& dir)
    {
        ...
        const F32 vert_enlargement = 1.f + (1.f - z) * 0.2f;
        const F32 half_reach = vert_enlargement * scale * HEAVENLY_BODY_FACTOR * disk_radius;
        return z + sqrtf(llmax(0.f, 1.f - z * z)) * half_reach > 0.f;
    }
}
/***************************************
        SkyTex
...
bool LLVOSky::updateGeometry(LLDrawable *drawable)
{
    ...
    if (ss_grace_band)
    {
        draw_sun  &= ssQuadClearsHorizon(mSunScale,  mSun.getDiskRadius(),  environment.getSunDirection());
        draw_moon &= ssQuadClearsHorizon(mMoonScale, mMoon.getDiskRadius(), environment.getMoonDirection());
    }
    else if (!(ss_atmo && ss_applier.isActive()))
    {
